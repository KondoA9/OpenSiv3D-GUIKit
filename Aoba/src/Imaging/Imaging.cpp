...
namespace s3d::aoba::Imaging {
    namespace Internal {
        Vec2 RotatePoint(const Vec2& pos, double degrees) {
            ...
            return {pos.x * cos(angle) - pos.y * sin(angle), pos.x * sin(angle) + pos.y * cos(angle)};
        }
    }
    ...
    Size GetSizeFitsTexture(const Size& textureSize, double degrees) {
        ...
        const auto right =
            Max(Max(rotatedLeftTop.x, rotatedRightTop.x), Max(rotatedLeftBottom.x, rotatedRightBottom.x));
        const auto top = Min(Min(rotatedLeftTop.y, rotatedRightTop.y), Min(rotatedLeftBottom.y, rotatedRightBottom.y));
        const auto bottom =
            Max(Max(rotatedLeftTop.y, rotatedRightTop.y), Max(rotatedLeftBottom.y, rotatedRightBottom.y));
