    ...
    void UISlider::initialize() {
        ...
        addEventListener<Event::Component::Slider::Sliding>(
            [this, handleRadius] {
                ...
                m_value          = Clamp(m_min
                                    + (m_max - m_min) * (Cursor::Pos().x - (layer().left() + handleRadius))
                                          / (layer().width() - handleRadius * 2),
                                m_min,
                                m_max);
        ...
        addEventListener<Event::Mouse::Hovered>([this] { m_uiHandle.backgroundColor.highlight(accentColor); }, true);
        ...
        m_uiHandle.setConstraint(LayerDirection::CenterX, [this, handleRadius] {
            return layer().left() + handleRadius
                   + (layer().width() - handleRadius * 2) * (m_value - m_min) / (m_max - m_min);
        });
