...
namespace s3d::aoba {
    ...
    void UIInputField::initialize() {
        ...
        addEventListener<Event::Component::UnFocused>(
            [this] {
    ...
    void UIInputField::draw() const {
        ...
        if (isFocused()) {
            if (m_isCursorVisible) {
                Line(m_cursorBeamPosX, m_fieldRect.y + 4_px, m_cursorBeamPosX, m_fieldRect.y + m_fieldRect.h - 4_px)
                    .draw(textColor);
            }
    ...
    String UIInputField::updateText() {
        ...
        {
            const auto fixedStr =
                editingString.removed_if([this](const char32& c) { return forbiddenCharacters.includes(c); });
    ...
    void UIInputField::updateCursorMovement() {
        ...
        if (operationCursorToLeft.isCalled() || operationCursorToRight.isCalled()) {
            ...
            if (!m_selectingByKeyboard) {
                // Reset selecting
                if (m_textSelected) {
                    m_cursorPos    = operationCursorToLeft.isCalled() ? Min(m_selectingCursorStart, m_cursorPos++)
                                                                      : Max(m_selectingCursorStart, m_cursorPos--);
                    m_textSelected = false;
                }
    ...
    void UIInputField::updateTextSelecting() {
        ...
        } else if (const Array<Input> activeKeys = Keyboard::GetAllInputs();
                   activeKeys.includes_if([](const Input key) { return key.down(); })) {
    ...
    void UIInputField::fireForbiddenCharsNotifier() {
        ...
        m_WarningTimeoutID = Core::SetTimeout([] { ui_Warning->hidden = true; }, 3000, false);
